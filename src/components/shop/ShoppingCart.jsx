...
export default function ShoppingCart() {
  ...
  if (!cartItem) return <div className="p-10 text-center">Your cart is empty</div>;
    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      <div className="border-t border-gray-300 py-6 grid grid-cols-4 items-center">
        <div className="flex gap-4 items-center">
          <img src={cartItem.image} alt={cartItem.name} className="w-24 h-28 object-cover" />
          <div>
            <h3 className="font-semibold">{cartItem.name}</h3>
            <button className="text-sm text-gray-500 underline hover:text-black">Remove</button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleDecrease} className="px-2 py-1 border border-gray-300">-</button>
          <span className="w-10 text-center border border-gray-300 py-1 text-sm">{quantity}</span>
          <button onClick={handleIncrease} className="px-2 py-1 border border-gray-300">+</button>
        </div>

        <div className="text-sm font-medium">${(cartItem.price * quantity).toFixed(2)}</div>
      </div>
